...
// This is to launch the puppeteer, which is a web scraper
async function scrapeProduct(url){
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
    await page.goto(url);

   //to evaluate the page to get the details from it
    var container = await page.evaluate(() => {
        //there are many classes and subclasses, due to which I have used try catch for each class
        //If one doen't work then it goes for another
        //If gets nothing then it will display nothing
        try{
            try{
                var titleNodeList = document.querySelectorAll(`.a-color-base.a-text-normal`);
                if (titleNodeList.length) {
                    //to get the required details by selecting their class and storing it in a list
                    ...
                    try{
                        for (var i=0; i< titleNodeList.length; i++){
                            titleLinkArray[i] = {
                                ...
                                //website from which the details are to be fetched
                                href: "https://www.amazon.com"+hrefNodeList[i].getAttribute("href"),
                                rating: 4,
                            };
                        }
                    }
                        ...
                        for (var i=0; i< titleNodeList.length; i++){
                            if (priceNodeList[i] != null) {
                                ...
                                hrefArray[i] = "https://www.amazon.com"+hrefNodeList[i].getAttribute("href");
                            }
                            else{
                                break;
                            }
...
var amazon = async function (details) {
    ...
    //searches the product user searched on our website
    new_url = 'https://www.amazon.com/s?k='+ details +'&ref=nb_sb_noss';
    //console.log(new_url);
